...
use crate::payload::{data_set::data_set_value, metric, property_value, template::parameter, DataType};
use crate::traits;
...
macro_rules! impl_wrapper_type_for_proto_value_type {
  ($new_type:ident, $wrapped_type:ty) => {
    ...
    impl $new_type {
      pub fn new(inner: $wrapped_type) -> Self { Self(inner) }
    }
...
impl DateTime {
  pub fn new(date_time: u64) -> Self { Self {date_time} }
...
fn bool_to_proto(val:bool) -> bool { val }
...
fn u32_to_proto(val: u32) -> u32 { val }
fn u64_to_proto(val: u64) -> u64 { val }
...
fn i32_to_proto(val: i32) -> u32 { u32::from_le_bytes(val.to_le_bytes()) }
fn i64_to_proto(val: i64) -> u64 { u64::from_le_bytes(val.to_le_bytes()) }
fn f32_to_proto(val: f32) -> f32 {val }
fn f64_to_proto(val: f64) -> f64 { val }
fn string_to_proto(val: String) -> String { val }
fn datetime_to_proto (val: DateTime) -> u64 { val.date_time }

fn proto_to_bool(val: bool) -> bool { val }
fn proto_to_u8(val: u32) -> u8 { val as u8 }
fn proto_to_u16(val: u32) -> u16 { val as u16 }
fn proto_to_u32(val: u32) -> u32 { val }
fn proto_to_u64(val: u64) -> u64 { val }
...
fn proto_to_i32(val: u32) -> i32 { i32::from_le_bytes(val.to_le_bytes()) }
fn proto_to_i64(val: u64) -> i64 { i64::from_le_bytes(val.to_le_bytes()) }
fn proto_to_f32(val: f32) -> f32 { val }
fn proto_to_f64(val: f64) -> f64 { val }
fn proto_to_string(val: String) -> String { val }
fn proto_to_datetime(val: u64) -> DateTime { DateTime{ date_time: val }}
...
fn u8_vec_to_proto(vec: Vec<u8>) -> Vec<u8> { vec }
fn proto_to_u8_vec(vec: Vec<u8>) -> Result<Vec<u8>, FromBytesError> { Ok(vec) }
...
{
  bools.iter().enumerate().fold(0u8, |acc, (i, b)| {
    acc | ((*b as u8) << 7 - i)
  })
}
...
fn bool_vec_to_proto(vec: Vec<bool>) -> Vec<u8> {
  ...
  for x in count.to_le_bytes() { out.push(x); }
  /* Pack bools into bytes */
  let chunks = vec.chunks_exact(8);
  let remainder = chunks.remainder();
  chunks.into_iter().for_each(|chunk| {
    out.push(pack_byte_with_bool(chunk))
  });
  if remainder.len() > 0 {out.push(pack_byte_with_bool(remainder))}
  out
}
...
fn proto_to_bool_vec(bytes: Vec<u8>) -> Result<Vec<bool>, FromBytesError> {
  ...
  if bool_count == 0 { return Ok(Vec::new()) }
...
fn proto_to_string_vec(vec: Vec<u8>) -> Result<Vec<String>, FromBytesError> {
  if let Some(last) = vec.last() {
    if *last != 0 {return Err(FromBytesError::InvalidFormat)}
  } 
  ...
  (
    ...
    impl TryFrom<$wrapping_type> for $type {
      ...
      fn try_from(value: $wrapping_type) -> Result<Self, Self::Error> {
        if let $proto_variant(v) = value.0 { Ok( $from_proto_fn(v)) } else { Err(FromValueTypeError::InvalidVariantType) }
      }
    }
...
impl_basic_type!(bool, [DataType::Boolean], metric::Value::BooleanValue, property_value::Value::BooleanValue, data_set_value::Value::BooleanValue, parameter::Value::BooleanValue, bool_to_proto, proto_to_bool);
impl_basic_type!(u8, [DataType::UInt8], metric::Value::IntValue, property_value::Value::IntValue, data_set_value::Value::IntValue, parameter::Value::IntValue, u8_to_proto, proto_to_u8);
impl_basic_type!(u16, [DataType::UInt16], metric::Value::IntValue, property_value::Value::IntValue, data_set_value::Value::IntValue, parameter::Value::IntValue, u16_to_proto, proto_to_u16);
impl_basic_type!(u32, [DataType::UInt32], metric::Value::IntValue, property_value::Value::IntValue, data_set_value::Value::IntValue, parameter::Value::IntValue, u32_to_proto, proto_to_u32);
impl_basic_type!(u64, [DataType::UInt64], metric::Value::LongValue, property_value::Value::LongValue, data_set_value::Value::LongValue, parameter::Value::LongValue, u64_to_proto, proto_to_u64);
impl_basic_type!(i8, [DataType::Int8], metric::Value::IntValue, property_value::Value::IntValue, data_set_value::Value::IntValue, parameter::Value::IntValue, i8_to_proto, proto_to_i8);
impl_basic_type!(i16, [DataType::Int16], metric::Value::IntValue, property_value::Value::IntValue, data_set_value::Value::IntValue, parameter::Value::IntValue, i16_to_proto, proto_to_i16);
impl_basic_type!(i32, [DataType::Int32], metric::Value::IntValue, property_value::Value::IntValue, data_set_value::Value::IntValue, parameter::Value::IntValue, i32_to_proto, proto_to_i32);
impl_basic_type!(i64, [DataType::Int64], metric::Value::LongValue, property_value::Value::LongValue, data_set_value::Value::LongValue, parameter::Value::LongValue, i64_to_proto, proto_to_i64);
impl_basic_type!(f32, [DataType::Float], metric::Value::FloatValue, property_value::Value::FloatValue, data_set_value::Value::FloatValue, parameter::Value::FloatValue, f32_to_proto, proto_to_f32);
impl_basic_type!(f64, [DataType::Double], metric::Value::DoubleValue, property_value::Value::DoubleValue, data_set_value::Value::DoubleValue, parameter::Value::DoubleValue, f64_to_proto, proto_to_f64);
impl_basic_type!(String, [DataType::String, DataType::Text], metric::Value::StringValue, property_value::Value::StringValue, data_set_value::Value::StringValue, parameter::Value::StringValue, string_to_proto, proto_to_string);
impl_basic_type!(DateTime, [DataType::DateTime], metric::Value::LongValue, property_value::Value::LongValue, data_set_value::Value::LongValue, parameter::Value::LongValue, datetime_to_proto, proto_to_datetime);
...
impl_vec_type_metric_value_conversions!(bool, [DataType::BooleanArray], bool_vec_to_proto, proto_to_bool_vec);
impl_vec_type_metric_value_conversions!(u8, [DataType::UInt8Array, DataType::Bytes], u8_vec_to_proto, proto_to_u8_vec);
impl_vec_type_metric_value_conversions!(u16, [DataType::UInt16Array], u16_vec_to_proto, proto_to_u16_vec);
impl_vec_type_metric_value_conversions!(u32, [DataType::UInt32Array], u32_vec_to_proto, proto_to_u32_vec);
impl_vec_type_metric_value_conversions!(u64, [DataType::UInt64Array], u64_vec_to_proto, proto_to_u64_vec);
impl_vec_type_metric_value_conversions!(i8, [DataType::Int8Array], i8_vec_to_proto, proto_to_i8_vec);
impl_vec_type_metric_value_conversions!(i16, [DataType::Int16Array], i16_vec_to_proto, proto_to_i16_vec);
impl_vec_type_metric_value_conversions!(i32, [DataType::Int32Array], i32_vec_to_proto, proto_to_i32_vec);
impl_vec_type_metric_value_conversions!(i64, [DataType::Int64Array], i64_vec_to_proto, proto_to_i64_vec);
impl_vec_type_metric_value_conversions!(f32, [DataType::FloatArray], f32_vec_to_proto, proto_to_f32_vec);
impl_vec_type_metric_value_conversions!(f64, [DataType::DoubleArray], f64_vec_to_proto, proto_to_f64_vec);
impl_vec_type_metric_value_conversions!(DateTime, [DataType::DateTimeArray], datetime_vec_to_proto, proto_to_datetime_vec);
impl_vec_type_metric_value_conversions!(String, [DataType::StringArray], string_vec_to_proto, proto_to_string_vec);
...
mod tests {
  ...
  */
  ...
  mod types {
    ...
    #[test]
    fn bool_array() {
      test_value_to_proto_value_and_back!(Vec<bool>, vec![false, false, true, true, false, true, false, false, true, true, false, true], BytesValue);
    }

    #[test]
    fn datetime_array() {
      test_value_to_proto_value_and_back!(Vec<DateTime>, vec![DateTime::new(1), DateTime::new(42)], BytesValue);
    }
  ...
  mod array_type_bytes_conversion {
    ...
    fn bool() {
      ...
      let bytes = bool_vec_to_proto(start.clone());
      assert_eq!(bytes, create_bool_bytes_vec(9, vec![0b1010_1110, 0b1000_0000]));
      assert_eq!(proto_to_bool_vec(bytes).unwrap(), start);

      let start = vec![false, false, true, true, false, true, false, false, true, true, false, true];
      let bytes = bool_vec_to_proto(start.clone());
      assert_eq!(bytes, create_bool_bytes_vec(12, vec![0b0011_0100, 0b1101_0000]));
      assert_eq!(proto_to_bool_vec(bytes).unwrap(), start);
    }
    ...
    fn test_standard_vec_u8_convertable_types() {
      ...
      let vec = vec![DateTime::new(0), DateTime::new(u64::MIN), DateTime::new(u64::MAX)];
      assert_eq!(vec, proto_to_datetime_vec(datetime_vec_to_proto(vec.clone())).unwrap());
    }
