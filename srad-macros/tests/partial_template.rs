use srad::types::{
    PartialTemplate, Template, TemplateError, TemplateInstance, TemplateMetadata, TemplateMetric,
    TemplateParameter,
};
...
fn test_template_instance_from_difference() {
    ...
    assert_eq!(
        ...
        Some(TemplateInstance {
            version: None,
            metrics: vec![TemplateMetric::new_template_metric("b".into(), 4),],
            parameters: vec![],
    ...
    assert_eq!(
        ...
        Some(TemplateInstance {
            version: None,
            metrics: vec![],
            parameters: vec![TemplateParameter::new_template_parameter("d".into(), 5),],
            template_ref: PartialTest::template_definition_metric_name()
        })
    );

    //test no difference
    assert_eq!(simple1.template_instance_from_difference(&simple1), None);
    let simple3 = simple1.clone();
    assert_eq!(simple3.template_instance_from_difference(&simple1), None);
}

#[test]
fn test_update_from_instance() {
    let mut simple1 = PartialTest {
        a: 1,
        b: 2,
        c: 3,
        d: 4,
    };
    let valid_update = || TemplateInstance {
        version: None,
        metrics: vec![TemplateMetric::new_template_metric("b".into(), 4)],
        parameters: vec![TemplateParameter::new_template_parameter("d".into(), 5)],
        template_ref: PartialTest::template_definition_metric_name(),
    ...
    assert!(matches!(
        simple1.update_from_instance(invalid_ref),
    ...
    assert!(matches!(
        simple1.update_from_instance(unknown_metric),
    ...
    assert!(matches!(
        simple1.update_from_instance(unknown_param),
...
fn test_template_instance_from_difference_nested() {
    ...
    assert_eq!(nested.template_instance_from_difference(&nested), None);
        ...
        Some(TemplateInstance {
            ...
            metrics: vec![
                ...
                TemplateMetric::new_template_metric(
                    ...
                    TemplateInstance {
                        version: None,
                        metrics: vec![TemplateMetric::new_template_metric("a".into(), 2_i32),],
                        parameters: vec![],
                        template_ref: PartialTest::template_definition_metric_name()
                    }
                ),
...
fn test_update_from_instance_nested() {
    let mut nested = PartialNestedTest {
        a: 2,
        b: 4,
        c: PartialTest {
            a: 1,
