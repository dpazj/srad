...
async fn reorder_rebirths() {
    ...
    broker
        .tx_event
        .send(srad_client::Event::Node(NodeMessage {
    ...
    let rebirth = timeout(
        Duration::from_millis(1000 + reorder_timeout),
        broker.rx_outbound.recv(),
    )
    .await
    .unwrap()
    .unwrap();
    ...
    assert_eq!(
        topic,
        NodeTopic::new("group1", srad_types::topic::NodeMessage::NCmd, "node1")
    );
