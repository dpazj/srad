...
use srad_types::{
    payload::StateBirthDeathCertificate,
    topic::{QoS, StateTopic, Topic, TopicFilter},
};
...
async fn app_states() {
    ...
    assert_eq!(
        payload,
        StatePayload::Online {
            timestamp: will_payload_timestamp
        }
    );
    ...
    broker
        .tx_event
        .send(srad_client::Event::State {
            host_id: app_id.into(),
            payload: StatePayload::Offline { timestamp: 0 },
        })
        .unwrap();
    let state_publish = timeout(Duration::from_secs(1), broker.rx_outbound.recv())
        .await
