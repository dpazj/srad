use srad_types::{constants::{BDSEQ, NODE_CONTROL_REBIRTH}, payload::{metric, DataType, Metric, Payload}, utils::timestamp, MetricValue};


pub fn new_nbirth_payload(metrics: Option<Vec<Metric>>) -> Payload
{
    let mut bdseq= Metric::new();
    bdseq.set_name(BDSEQ.into())
        .set_datatype(DataType::Int64)
        .set_timestamp(timestamp())
        .set_value(MetricValue::from(0i64).into());

    let mut node_control_rebirth= Metric::new();
    node_control_rebirth.set_name(NODE_CONTROL_REBIRTH.into())
