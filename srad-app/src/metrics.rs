...
pub fn create_metric_tokens_from_birth_metrics(metrics: Vec<payload::Metric>) -> Result<Vec<(MetricToken, MetricDetails)>,()> {
    ...
    for x in metrics {
        ...
        let token = MetricToken::new(name.clone(), alias, datatype);
        let details = metric_details_try_from_payload_metric!(x)?;
